...
str=input("请输入(train表示训练，predict表示测试)：")
if __name__ =='__main__' and str=='train':
    # 第一次遍历图片目录是为了获取图片总数
    input_count = 0
    for i in range(0,NUM_CLASSES):
        dir = './digit_images/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
    ...
    val_count = 0
    for i in range(0,NUM_CLASSES):
        dir = './digit_validation/%s/' % i           # 这里可以改成你自己的图片目录，i为分类标签
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
    print("valcount=", val_count)
    ...
    with tf.Session() as sess:
        ...
        for it in range(iterations):
            ...
            if it%5 == 0:
                ...
                if iterate_accuracy >= 0.99 or it >= iterations:
...
if __name__ =='__main__' and str=='predict':
    ...
    with tf.Session() as sess:
        ...
        for n in range(0,18):
            ...
            # img = Image.open(path)
            img = Image.open('./divide/f8/%s.png'%(n))
            width = img.size[0]
            height = img.size[1]
            ...
            predict_list.append([max1_index,max1])
            probality.append(max1)
            print ("概率：  [%s %0.2f%%]    [%s %0.2f%%]    [%s %0.2f%%]" % ([max1_index],max1*100, [max2_index],max2*100, [max3_index],max3*100))

        probality.sort(reverse = True)
        print (probality)
        max7 = probality[7]
        final_list = ''
        print(max7)
        for pair in predict_list:
            if pair[1] >max7:
                final_list += LETTERS_DIGITS[pair[0]]
        print ("车牌编号是: 【%s】" % final_list)
